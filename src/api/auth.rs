...
pub mod auth {
    ...
    impl Ticket {
        ...
        pub fn cancel(&mut self) {
            ...
            log_function_call("auth.Ticket.cancel", &[], &"()");
        }

        #[napi]
        pub fn get_bytes(&self) -> Buffer {
            let result = self.data.clone().into();
            log_function_call("auth.Ticket.get_bytes", &[], &"Buffer");
            result
        }
    }
    ...
    pub async fn get_session_ticket_with_steam_id(
        ...
        let result = get_session_ticket(
            NetworkingIdentity::new_steam_id(SteamId::from_raw(steam_id64.get_u64().1)),
            timeout_seconds,
        )
        .await;
        log_function_call(
            "auth.get_session_ticket_with_steam_id",
            &[&steam_id64, &timeout_seconds],
            &result.is_ok(),
        );
        result
    }
    ...
    pub async fn get_auth_ticket_for_web_api(
        ...
        let result = (|| async {
            ...
            let callback = client
                .register_callback(move |ticket_for_webapi_response: TicketForWebApiResponse| {
            ...
            let timeout_seconds_u64 = u64::from(timeout_seconds.unwrap_or(10));
            let result =
                tokio::time::timeout(std::time::Duration::from_secs(timeout_seconds_u64), rx)
                    .await;
        ...
        log_function_call(
            "auth.get_auth_ticket_for_web_api",
